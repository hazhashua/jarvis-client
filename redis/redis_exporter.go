...
import (
	...
	digoredis "github.com/gomodule/redigo/redis"
...
func NewRedisExporter(redisURI string, host string, ip string, cluster string, opts Options) (*Exporter, error) {
	...
	connectedClientsLabels := []string{"name", "created_at", "idle_since", "flags", "db", "omem", "cmd", "host", "cluster", "exporter_host", "exporter_ip"}
	...
	if e.options.Registry != nil {
		...
		redis_handler := promhttp.HandlerFor(e.options.Registry, promhttp.HandlerOpts{ErrorHandling: promhttp.ContinueOnError})
		http.Handle(e.options.MetricsPath, redis_handler)
...
func (e *Exporter) scrapeRedisHost(ch chan<- prometheus.Metric) error {
	...
	infoAll, err := digoredis.String(doRedisCmd(c, "INFO", "ALL"))
	if err != nil || infoAll == "" {
		log.Debugf("Redis INFO ALL err: %s", err)
		infoAll, err = digoredis.String(doRedisCmd(c, "INFO"))
		if err != nil {
