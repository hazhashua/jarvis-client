...
func (e *Exporter) extractInfoMetrics(ch chan<- prometheus.Metric, info string, dbCount int, redis_config RedisConfig) {
	...
	for _, line := range lines {
		...
		case "Keyspace":
			if keysTotal, keysEx, avgTTL, keysCached, ok := parseDBKeyspaceString(fieldKey, fieldValue); ok {
				...
				e.registerConstMetricGauge(ch, "db_keys", keysTotal, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, dbName)
				e.registerConstMetricGauge(ch, "db_keys_expiring", keysEx, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, dbName)
				if keysCached > -1 {
					e.registerConstMetricGauge(ch, "db_keys_cached", keysCached, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, dbName)
				}
				if avgTTL > -1 {
					e.registerConstMetricGauge(ch, "db_avg_ttl_seconds", avgTTL, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, dbName)
				}
	...
	for dbIndex := 0; dbIndex < dbCount; dbIndex++ {
		...
		if _, exists := handledDBs[dbName]; !exists {
			e.registerConstMetricGauge(ch, "db_keys", 0, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, dbName)
			e.registerConstMetricGauge(ch, "db_keys_expiring", 0, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, dbName)
		}
	}

	e.registerConstMetricGauge(ch, "instance_info", 1,
		redis_config.Cluster.Name,
		e.redis_config.Cluster.Hosts[e.current_redis_idx],
		e.redis_config.Cluster.Ips[e.current_redis_idx],
...
func (e *Exporter) handleMetricsReplication(ch chan<- prometheus.Metric, masterHost string, masterPort string, fieldKey string, fieldValue string, redis_config RedisConfig) bool {
	...
	if fieldKey == "master_link_status" {
		if fieldValue == "up" {
			e.registerConstMetricGauge(ch, "master_link_up", 1, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, masterHost, masterPort)
		} else {
			e.registerConstMetricGauge(ch, "master_link_up", 0, redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, masterHost, masterPort)
		}
		return true
	}
	switch fieldKey {
	...
	case "master_last_io_seconds_ago", "slave_repl_offset", "master_sync_in_progress":
		...
		e.registerConstMetricGauge(ch, fieldKey, float64(val), redis_config.Cluster.Name, e.redis_config.Cluster.Hosts[e.current_redis_idx], e.redis_config.Cluster.Ips[e.current_redis_idx], redis_config.Cluster.ScrapeHost, redis_config.Cluster.ScrapeIp, masterHost, masterPort)
		return true
	}
...
func (e *Exporter) handleMetricsCommandStats(ch chan<- prometheus.Metric, fieldKey string, fieldValue string, redis_config RedisConfig) (cmd string, calls float64, usecTotal float64) {
	...
	if err == nil {
		...
		if extendedStats {
			e.registerConstMetric(ch, "commands_rejected_calls_total", rejectedCalls, prometheus.CounterValue,
				redis_config.Cluster.Name,
				e.redis_config.Cluster.Hosts[e.current_redis_idx],
				e.redis_config.Cluster.Ips[e.current_redis_idx],
				redis_config.Cluster.ScrapeHost,
				redis_config.Cluster.ScrapeIp,
				cmd)
			e.registerConstMetric(ch, "commands_failed_calls_total", failedCalls, prometheus.CounterValue,
				redis_config.Cluster.Name,
				e.redis_config.Cluster.Hosts[e.current_redis_idx],
				e.redis_config.Cluster.Ips[e.current_redis_idx],
				redis_config.Cluster.ScrapeHost,
