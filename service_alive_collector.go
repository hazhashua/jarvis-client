...
import (
	microservice "alive_exporter/micro_service"
...
type DatsourceAlive struct {
	// id      cluster         name    datasource_type         metric_timestamp        metric_value
	ServiceName  *string
	ChildService *string
	ClusterName  *string
	IP           *string
...
// func (collector *serviceAliveCollector) Describe(ch chan<- *prometheus.Desc) {
//Update this section with the each metric you create for a given collector
// 	ch <- collector.aliveMetric
// }
...
func (collector *serviceCollector) Collect(ch chan<- prometheus.Metric) {
	...
	// if 1 == 1 {
	// 	metricValue = float64(getAliveValue(collector.channel).MetricValue)
	// }

	// alives := getAliveValue(collector.channel)
...
func getServices(bytes []byte) {
	// 获取k8s service的返回内容
	microservice.UnmarshalAPIV1Services([]byte("hello go"))
