...
func (e *MysqlExporter) Collect(ch chan<- prometheus.Metric) {
	...
	for idx, table := range tableTables {
		...
		ch <- prometheus.MustNewConstMetric(e.tableInfos[idx], prometheus.GaugeValue, 1, table.TableSchema, table.TableName, fmt.Sprintf("%d", table.TableRows), fmt.Sprintf("%d", table.DataSize), fmt.Sprintf("%d", table.IndexSize), mysqlConfig.Cluster.Name, mysqlConnector.Host)
	}
